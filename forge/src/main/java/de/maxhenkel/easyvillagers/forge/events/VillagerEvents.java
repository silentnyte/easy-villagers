...
public class VillagerEvents {
    ...
    public void onClick(PlayerInteractEvent.EntityInteract event) {
        ...
        if (!EasyVillagersForge.CLIENT_CONFIG.enableRightClickPickup.get()) {
            return;
        }
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (!EasyVillagersForge.PICKUP_KEY.consumeClick()) {
            return;
        }
        ...
        EasyVillagersForge.SIMPLE_CHANNEL.sendToServer(new MessagePickUpVillager(pointedEntity.getUUID()));
    }
