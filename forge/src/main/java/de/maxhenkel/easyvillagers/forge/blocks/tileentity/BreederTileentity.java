...
import de.maxhenkel.corelib.forge.net.NetUtils;
...
public class BreederTileentity extends FakeWorldTileentity implements IServerTickableBlockEntity {
    ...
    public void tickServer() {
        ...
        if (level.getGameTime() % EasyVillagersForge.SERVER_CONFIG.breedingTime.get() == 0) {
            tryBreed();
        }
    }
    ...
    public void spawnParticles() {
        if (level.isClientSide) {
            ...
            EntityUtils.forEachPlayerAround((ServerLevel) level, worldPosition, 128, playerEntity -> NetUtils.sendTo(EasyVillagersForge.SIMPLE_CHANNEL, playerEntity, msg));
        }
    }
