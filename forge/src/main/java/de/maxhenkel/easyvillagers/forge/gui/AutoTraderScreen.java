...
public class AutoTraderScreen extends ScreenBase<AutoTraderContainer> {
    ...
    public static final ResourceLocation BACKGROUND = new ResourceLocation(EasyVillagers.MOD_ID, "textures/gui/container/auto_trader.png");
    ...
    protected void init() {
        ...
        addRenderableWidget(new Button(leftPos + 8, topPos + 19, 16, 20, TextComponent.EMPTY, button -> {
            EasyVillagersForge.SIMPLE_CHANNEL.sendToServer(new MessageSelectTrade(false));
        }));

        addRenderableWidget(new Button(leftPos + imageWidth - 16 - 8, topPos + 19, 16, 20, TextComponent.EMPTY, button -> {
            EasyVillagersForge.SIMPLE_CHANNEL.sendToServer(new MessageSelectTrade(true));
        }));
    }
