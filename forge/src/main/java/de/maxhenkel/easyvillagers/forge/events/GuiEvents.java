package de.maxhenkel.easyvillagers.forge.events;

import de.maxhenkel.easyvillagers.forge.entity.EasyVillagerEntity;
import de.maxhenkel.easyvillagers.forge.ClientConfig;
import de.maxhenkel.easyvillagers.forge.EasyVillagersForge;
import de.maxhenkel.easyvillagers.forge.gui.CycleTradesButton;
...
public class GuiEvents {
    ...
    public void onOpenScreen(GuiScreenEvent.InitGuiEvent.Post event) {
        if (!EasyVillagersForge.SERVER_CONFIG.tradeCycling.get()) {
            return;
        }
        ...
        ClientConfig.CycleTradesButtonLocation loc = EasyVillagersForge.CLIENT_CONFIG.cycleTradesButtonLocation.get();
        ...
        event.addWidget(new CycleTradesButton(posX, merchantScreen.getGuiTop() + 8, b -> {
            EasyVillagersForge.SIMPLE_CHANNEL.sendToServer(new MessageCycleTrades());
        }, merchantScreen));
    }
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (event.getKey() != EasyVillagersForge.CYCLE_TRADES_KEY.getKey().getValue() || event.getAction() != 0) {
            return;
        }
