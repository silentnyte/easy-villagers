...
public class AutoTraderTileentity extends TraderTileentityBase implements ITickableBlockEntity {
    ...
    public void tick() {
        ...
        if (level.getGameTime() % EasyVillagersForge.SERVER_CONFIG.autoTraderCooldown.get() != 0) {
            return;
        }
    ...
    protected long calculateNextRestock() {
        return EasyVillagersForge.SERVER_CONFIG.autoTraderMinRestockTime.get() + level.random.nextInt(Math.max(EasyVillagersForge.SERVER_CONFIG.autoTraderMaxRestockTime.get() - EasyVillagersForge.SERVER_CONFIG.autoTraderMinRestockTime.get(), 1));
    }
